...
Port io_exec(Net* net, Book* book, Port argm) {
  ...
  if (pid == 0) {
    ...
    execvp(args.strs[0], &args.strs[1]);
  ...
  // need to compute the lengths of these strings so they can be turned into
  // Bytes and injected back into nets
  res_stdout = read_all(stdout_pipe[0]);
  res_stderr = read_all(stderr_pipe[0]);
  ...
  Tup res;
  // TODO: add the results to this tuple
